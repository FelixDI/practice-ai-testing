...
#### 作用域优先级
- **`module` 级优先**：凡是多个用例共用的前置数据（注册用户、创建购物车、预置发票），必须使用 `scope="module"`，整个测试文件复用同一个实例
- **`session` 级谨慎使用**：仅用于全局不变的配置（如 base_url、固定测试账号），避免跨文件数据污染
- **`function` 级最小化**：仅用于需要完全数据隔离的场景（如测试用户注销后重新登录、修改核心状态的写操作）
