...
>
> ```python
> # ✅ fixture 中调用 conftest 共享工具获取商品 ID
> # tests/ui/conftest.py 已提供 fetch_valid_product_id()
> from tests.ui.conftest import fetch_valid_product_id
>
> # ❌ VALID_ID = "01KWR3GEF..."  # 迟早过期
> ```
...
3. **Fallback 仅限本地** —— `os.getenv("TEST_USER_EMAIL")` 的 fallback 值仅供本地开发。Jenkins 必须在 Setup 阶段从 `config.py` 读取 `JENKINS_TEST_EMAIL` 常量并注入为环境变量，注入失败就停掉 pipeline。禁止降级到本地 fallback
